...
namespace Training.DAL
{
    public class ReservationRepository : IReservationRepository
    {
        ...
        {
            _trainingDbContext.Reservations.Add(reservation);
        }
        public Reservation GetById(Guid reservationId)
        {
            return _trainingDbContext.Reservations.FirstOrDefault(x => x.Id == reservationId);
        }
        public IEnumerable<Reservation> GetByUser(User user)
        {
            return _trainingDbContext.Reservations.Where(x => x.User == user);
        }
